...
fn population_construction_stores_neuron_count() {
    ...
    assert_eq!(population.cumulative_spike_energy_nj(), 0);
}
...
fn recording_spikes_adds_exact_counts_and_energy() {
    ...
    population.record_spikes(50, 10).expect("valid spike batch");

    assert_eq!(population.total_spike_count(), 50);
    assert_eq!(population.cumulative_spike_energy_nj(), 50 * 10);
    ...
    assert_eq!(population.cumulative_spike_energy_nj(), 75 * 10);
}
